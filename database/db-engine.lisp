...
(defun make-patterns (smarts smiles-map data &optional (range '(-1.0 1.0)))
  (let ((patterns (make-list (length smiles-map)))
    ...
    (let* ((min (first range))
	   (max (second range))
	   (neg (count nil grep))
	   (pos (count nil grep :test-not #'eql))
	   new-min new-max)
      (setf new-min (if (> neg pos) (* min (/ pos neg)) min))
      (setf new-max (if (> neg pos) max (* max (/ neg pos))))
;      (when (> new-min -0.3) (setf new-min -0.3))
;      (when (< new-max 0.3) (setf new-max 0.3))
      (setf new-min (* (signum new-min) (sqrt (abs new-min))))
      (setf new-max (* (signum new-max) (sqrt (abs new-max))))
      (dolist (p patterns)
	(setf (aref (second p) 0)
	      (if (= (aref (second p) 0) min)
		  new-min new-max))))
